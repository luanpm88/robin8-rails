    ...
    return (
      <div className="brand-activity-card brand-activity-card-detail">
        <div className="brand-activity-content">
          ...
          <small className="date">最后更新: { DateFormater(campaign.get("updated_at")) }, 按照{showCampaignTypeText(campaign.get("per_budget_type"))}奖励 </small>
          <div className="summary">{ campaign.get("desc") }</div>
          <a href="#" className="link">{ campaign.get("url") }</a>
          <ul className="stat-info grid-3">
            <li><span className="txt">起止时间</span><small className="date">{ DateFormater(campaign.get("created_at")) } - { DateFormater(campaign.get("deadline ")) }</small></li>
