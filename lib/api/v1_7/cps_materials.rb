      ...
      resources :cps_materials do
        ...
        get '/' do
          if params[:category].present?
            cps_materials = CpsMaterial.where(:category => params[:category_name]).order("position desc").page(params[:page]).per_page(10)
          else
            cps_materials = CpsMaterial.order("position desc").page(params[:page]).per_page(10)
          end
          present :error, 0
