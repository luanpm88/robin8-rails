  ...
  Visitor.class_eval do
    def visit_Ransack_Nodes_Sort(object)
      ...
      if object.valid?
        ...
        { name: object.name, dir: object.dir }
      end
    end
  end
      ...
      Context.class_eval do
        def evaluate(search, opts = {})
          ...
          if search.sorts.any?
            ...
            viz.accept(search.sorts).each do |sort|
              if sort.is_a?(Hash)
                relation = relation.send("sort_by_#{sort[:name]}", sort[:dir].upcase)
              else
                relation = relation.order(sort)
              end
            end
          end
