!function($, wysi) {
    ...
    var defaultOptions = $.fn.wysihtml5.defaultOptions = {
        ...
        stylesheets: ["/assets/lib/bootstrap3-wysihtml5-editor.css"],
        locale: "en"
    };
...
(function(wysihtml5) {
    ...
    wysihtml5.commands.insertVideo = {
        ...
        exec: function(composer, command, src, format) {
            ...
            video.height = '315px';
            video.border = '0px';
            // wrapper.className = "embed-responsive embed-responsive-4by3";
            video.className = "embed-responsive-item";
            wrapper.appendChild(video);

            var embedUrl = false;
            if (/^(https?\:\/\/(www\.)?(m\.)?youtube.com\/)/i.test(src) || /^(https?\:\/\/(www\.)?youtu.be\/)/i.test(src)) {
                var regExp = /^.*((youtu.be\/)|(v\/)|(\/u\/\w\/)|(embed\/)|(watch\?))\??v?=?([^#\&\?]*).*/;
            ...
            } else if(/^(https?\:\/\/(www\.)?(player\.)?vimeo.com\/)/i.test(src)) {
                embedUrl = '//player.vimeo.com/video/' + src.split("/").pop() + "?title=0&byline=0&portrait=0&badge=0";
            } else if(/^(https?\:\/\/(www\.)?dailymotion.com\/)/i.test(src)) {
                embedUrl = src.replace("/video/", "/embed/video/");
            } else if(/^(https?\:\/\/(www\.)?vbox7.com\/play:)/i.test(src)) {
                embedUrl = src.replace("/play:", "/emb/external.php?vid=");
            } else if(/^(https?\:\/\/(www\.)?metacafe.com\/w(atch)?)/i.test(src)) {
                embedUrl = src.replace(/\/w(atch)?\//, "embed");
            }
