class CampaignInvite < ActiveRecord::Base
    ...
    if campaign.status == 'executing'
      ...
      ActiveRecord::Base.transaction do
        ...
        if campaign.is_click_type?  || campaign.is_cpa_type?
          kol.income(self.avail_click * campaign.actual_per_action_budget(false), 'campaign', campaign, campaign.user)
          ...
          kol.income(campaign.actual_per_action_budget(false), 'campaign', campaign, campaign.user)
  ...
  def earn_money
    ...
    if campaign.is_click_type? or campaign.is_cpa_type?
      (get_avail_click * campaign.actual_per_action_budget(false)).round(2)       rescue 0
    else
      campaign.actual_per_action_budget(false).round(2) rescue 0
    end
  end
