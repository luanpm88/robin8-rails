class Stream < ActiveRecord::Base
  ...
  private
    ...
    def decrease_feature_number
      af = needed_user.current_user_features.media_monitoring.available.joins(:product).where(products: {is_package: false}).first
      uf = af.nil? ? needed_user.current_user_features.media_monitoring.available.first : af
    ...
    def increase_feature_number
      af = needed_user.current_user_features.media_monitoring.available.joins(:product).where(products: {is_package: false}).first
      uf = af.nil? ? needed_user.current_user_features.media_monitoring.used.first : af
      return false if uf.blank?
      uf.available_count += 1
      uf.save
    end
