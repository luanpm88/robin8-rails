class KolInfluenceValueHistory < ActiveRecord::Base
  ...
  def self.get_auto_history(kol_uuid, kol_id)
    ...
    if kol_id
      history =  KolInfluenceValueHistory.select("max(influence_score), created_at").where(:kol_id => kol_id).group("DATE(created_at), kol_id").order("id desc").limit(HistorySize)
    else
      history =  KolInfluenceValueHistory.select("max(influence_score), created_at").where(:kol_uuid => kol_uuid).group("DATE(created_at), kol_uuid").order("id desc").limit(HistorySize)
    end
