class Pitch < ActiveRecord::Base
  ...
  def decrease_feature_number
    af = needed_user.current_user_features.smart_release.available.joins(:product).where(products: {is_package: false}).first
    uf = af.nil? ? needed_user.current_user_features.smart_release.available.first : af
  ...
  def increase_feature_number
    af = needed_user.current_user_features.smart_release.used.joins(:product).where(products: {is_package: false}).first
    uf = af.nil? ? needed_user.current_user_features.smart_release.used.first : af
    return false if uf.blank?
    uf.available_count += 1
    uf.save
  end
