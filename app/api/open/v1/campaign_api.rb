    ...
    class CampaignAPI < Base
      ...
      resource :campaigns do
        ...
        params do
          ...
          requires :screenshot_url,    type: String
        ...
        post "campaign_click_stats" do
          @campaign = current_user.campaigns.find(params[:id])
          @campaign_show = CampaignShow.where(created_at: (params[:starttime]..params[:endtime])).where(campaign_id: @campaign.id).
            # @campaign_show = CampaignShow.where(created_at: (params[:starttime]..params[:endtime])).
            group("date_format(created_at, 'YYYY-MM-DD')").
            select("date_format(created_at, 'YYYY-MM-DD') as date, sum(status) as avail_click, count(status) as total_click")
          present :success, true
