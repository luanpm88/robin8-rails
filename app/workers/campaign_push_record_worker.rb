class CampaignPushRecordWorker
  ...
  def perform(campaign_id, type=nil, kols=[])
    ...
    if type == 'common'
      if @campaign.is_invite_type? && @campaign.get_social_account_related_kol_ids #针对特邀活动，指定了特定的社交账号，通过社交账号来找到KOL
        @campaign.get_social_account_related_kol_ids.each_slice(1000) do |kol_ids|
          inserts = []
        ...
        @campaign.get_specified_kol_ids.each_slice(1000) do |kol_ids|
          inserts = []
          kol_ids.each do |kol_id|
      ...
      @campaign.get_append_kol_ids.each_slice(1000) do |kol_ids|
        inserts = []
        kol_ids.each do |kol_id|
      ...
      if kols.present?
        kols.each_slice(1000) do |kol_ids|
          inserts = []
          kol_ids.each do |kol_id|
  ...
  def save_matched_kols
    @campaign.get_matching_kol_ids.each_slice(1000) do |kol_ids|
      inserts = []
      kol_ids.each do |kol_id|
  ...
  def save_removed_kols_by_specified_campaign # 参与指定活动的kol
    @campaign.get_remove_kol_ids_of_campaign_by_target.each_slice(1000) do |kol_ids|
      inserts = []
      kol_ids.each do |kol_id|
  ...
  def save_specify_removed_kols #指定的kol
    @campaign.get_remove_kol_ids_by_target.each_slice(1000) do |kol_ids|
      inserts = []
      kol_ids.each do |kol_id|
  ...
  def save_black_list_kols  # 黑名单中的kol
    @campaign.get_black_list_kols.each_slice(1000) do |kol_ids|
      inserts = []
      kol_ids.each do |kol_id|
  ...
  def save_today_receive_three_times_kols # 一天接了3单的Kol
    @campaign.today_receive_three_times_kol_ids.each_slice(1000) do |kol_ids|
      inserts = []
      kol_ids.each do |kol_id|
  ...
  def save_frequent_push_kols # 推送过于频繁
    @campaign.three_hours_had_receive_kol_ids.each_slice(1000) do |kol_ids|
      inserts = []
      kol_ids.each do |kol_id|
