ActiveAdmin.register CampaignInvite do
  ...
  index do
    ...
    actions do |my_resource|
      if my_resource.campaign.status != "settled" && my_resource.img_status == "pending" && my_resource.screenshot.present? && (my_resource.campaign.actual_deadline_time > Time.now - 1.days || my_resource.campaign.actual_deadline_time.nil?)
