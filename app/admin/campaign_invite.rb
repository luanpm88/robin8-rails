ActiveAdmin.register CampaignInvite do
  ...
  before_filter :only => [:index] do
    params['q'] = {:campaign_status_eq => 'executed', :status_eq => 'executed', :img_status_eq => 'pending', :screenshot_present => '1'}
  end
  ...
  index do
    ...
    actions do |my_resource|
      if my_resource.campaign.status == "executed" && my_resource.img_status == "pending" && my_resource.screenshot.present? && (my_resource.campaign.deadline > Time.now - 7.days)
