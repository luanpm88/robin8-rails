...
class MarketingDashboard::StasticDatasController < MarketingDashboard::BaseController
  ...
  def new_kol
    ...
    day_count.times.each do |index|
      @collect << {:date => (today - index.days), :bind_mobiles => (@bind_mobiles[index].count rescue nil), :unbind_mobiles => (@unbind_mobiles[index].count rescue nil), :wechat => (@wechat[index].count rescue nil),
