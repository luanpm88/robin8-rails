...
class MarketingDashboard::AlipayOrdersController < MarketingDashboard::BaseController
    ...
    respond_to do |format|
      ...
      format.csv {
        csv_string = CSV.generate do |csv|
          csv << ["ID", "品牌主ID", "品牌昵称", "品牌主手机", "销售人员", "下单时间", "订单号", "支付宝订单号", "充值金额", "税费", "充值状态", "查看流水（若未付款则为空）"]
          @alipay_orders.each do |c|
            csv << [c.id, c.user_id, c.id, c.user.smart_name, c.user.mobile_number,  c.user.seller.name, c.created_at.strftime("%Y-%m-%d %H:%M:%S"), c.trade_no, c.alipay_trade_no, c.credits, c.tax, c.status, "查看流水" ]
          end
        end
  ...
  def campaigns
    ...
    respond_to do |format|
      ...
      format.csv {
        csv_string = CSV.generate do |csv|
          csv << ["活动ID", "活动名称", "品牌主ID", "品牌名称", "订单号", "支付宝订单号", "支付金额", "支付状态", "下单时间", "销售人员"]
          @campaigns.each do |c|
