class WechatKolCampaignController < ApplicationController
  ...
  def kol_register
    ...
    Rails.logger.wechat_kol_campaign.info "--kol_register @app_download_url : #{@app_download_url}"
    @tag = params[:tag]
    render :layout => false
  end
  ...
  def sms_request
    ...
    Rails.logger.wechat_kol_campaign.info "--sms_request: sms count #{send_count}"

    sms_client = YunPian::SendRegisterSms.new(mobile_number)

    res = sms_client.send_sms  rescue {}
    Rails.logger.wechat_kol_campaign.info "--sms_request: send sms code #{res}"
    render json: res
  end
  ...
  def kol_create
    ...
    Rails.logger.wechat_kol_campaign.info "--kol_create: kol_exists: #{kol_exists}"
    if kol_exists
      campaign_id = params[:campaign_id]
      Rails.logger.wechat_kol_campaign.info "campaign ID: #{campaign_id}"
      campaign = Campaign.find(campaign_id) rescue nil
      token = tmp_token kol_exists, campaign
      cookies[:_robin8_wechat_kol_campaign] = token
      cookies[:_robin8_wechat_kol_campaign_id] = kol_exists.id
      Rails.logger.wechat_kol_campaign.info "--kol_create: cookies[:_robin8_wechat_kol_campaign] : #{token}"
      Rails.logger.wechat_kol_campaign.info "--kol_create: cookies[:_robin8_wechat_kol_campaign_id] : #{kol_exists.id}"

      cache_key = "wechat-campaign-token-#{kol_exists.id}-#{campaign_id}"
      Rails.cache.write(cache_key, token , :expires_in => 1.hour)
      Rails.logger.wechat_kol_campaign.info "--kol_create: cache_key: #{cache_key}"

      Rails.logger.wechat_kol_campaign.info "--kol_create: campaign status: #{campaign.status}"
      ...
      return render json: {url: wechat_kol_campaign_campaign_details_path(campaign_id: campaign_id) }
    else
      ...
      if kol.save
        ...
        cookies[:_robin8_wechat_kol_campaign] = token
        cookies[:_robin8_wechat_kol_campaign_id] = kol.id
        Rails.logger.wechat_kol_campaign.info "--kol_create: cookies[:_robin8_wechat_kol_campaign] : #{token}"
        Rails.logger.wechat_kol_campaign.info "--kol_create: cookies[:_robin8_wechat_kol_campaign_id] : #{kol.id}"
        ...
        Rails.logger.wechat_kol_campaign.info "--kol_create: campaign not found"
  ...
  def campaign_details
    client_token = cookies[:_robin8_wechat_kol_campaign]
    kol_id = cookies[:_robin8_wechat_kol_campaign_id]
    Rails.logger.wechat_kol_campaign.info "--campaign_details: client_token #{client_token}"
    Rails.logger.wechat_kol_campaign.info "--campaign_details: kol_id #{kol_id}"
    campaign_id = params[:campaign_id]
    campaign = Campaign.find(campaign_id)
    Rails.logger.wechat_kol_campaign.info "--campaign_details: campaign_id #{campaign_id}"

    cache_key = "wechat-campaign-token-#{kol_id}-#{campaign_id}"
    cached_token = Rails.cache.fetch(cache_key)
    Rails.logger.wechat_kol_campaign.info "--campaign_details: cache_key #{cache_key}"

    @campaign_info_box = campaign.per_budget_type == 'cpt' ? campaign.remark : nil rescue nil
    Rails.logger.wechat_kol_campaign.info "--campaign_details: @campaign_info_box #{@campaign_info_box}"
    ...
    if client_token and cached_token and client_token == cached_token
      ...
      Rails.logger.wechat_kol_campaign.info "--campaign_details: campaign_invite.id #{@campaign_invite.id}"
      if @campaign_invite
      ...
      redirect_to wechat_kol_campaign_campaign_page_url(campaign_id: campaign_id)
      return nil
    end
  ...
  def tmp_token kol, campaign
    password = "#{kol.id}-#{campaign.id}-wechat-kol-campaign-Robin8"
    Rails.logger.wechat_kol_campaign.info "--tmp_token: password #{password}"
