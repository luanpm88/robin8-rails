class PagesController < ApplicationController
  ...
  def home
    if user_signed_in? && !current_user.active_subscription.blank?
      ...
      to_verify_count = current_kol.campaign_invites.where(status: 'finished', img_status: 'pending', screenshot: nil).joins(:campaign).where('campaign_invites.avail_click > 0 AND campaigns.deadline > ?', Time.now - Campaign::SettleWaitTimeForKol).count
