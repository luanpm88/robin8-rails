class RegisteredInvitationsController < ApplicationController
  ...
  def create
    ...
    if mobile_number[0] == '0'
      mobile_number = '+84' + mobile_number[1..-1]
    elsif mobile_number[0] != '+'
      mobile_number = '+' + mobile_number
    end

    verify_code = $redis.get(mobile_number)
    return render json: {error: "短信验证码错误" } unless verify_code == params[:sms_code]
    return render json: {error: "手机号已经被注册" } if Kol.where(mobile_number: mobile_number).exists?
    ...
    @invitation = RegisteredInvitation.where(mobile_number: mobile_number).first_or_create(
      inviter_id: @kol.id,
      status: "pending"
    )
    ...
    puts @invitation.errors.to_json
  end
  ...
  def sms
    ...
    mobile_number = params[:mobile_number]    
    return render json: {error: t("invite.mobile_number_required")} if mobile_number.blank?
    if mobile_number[0] == '0'
      mobile_number = '+84' + mobile_number[1..-1]
    elsif mobile_number[0] != '+'
      mobile_number = '+' + mobile_number
    ...
    return render json: {error: t("invite.mobile_number_registered")} if number_existed
    ...
    Rails.logger.sms_spider.info t("invite.effective_amount_exceeded") + ": #{send_count}"

    sms_client = YunPian::SendInternationalSms.new(mobile_number)
